...
export default function Test6() {
  ...
  // ambient playback is handled by SettingsNavigator via HTMLAudio
...
// mapping is dynamic now; SettingsNavigator will update this
  ...
  const handleSettingChange = (setting, index) => {
    ...
    Object.entries(setting.colorToMap || {}).forEach(([color, val]) => {
      newMap[color] = `${val.note}${val.octave}`;
    });
  ...
  const getNoteForCell = (r, g, b, x, y) => {
    ...
    // mapped already contains note+octave like "G3"
    return mapped;
  };
  ...
  const startAudio = async () => {
    ...
    // ambient playback is automatic via SettingsNavigator's HTMLAudio element
    startAnimation();
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', backgroundColor: 'black', padding: '16px' }}>
      <div style={{ marginBottom: '16px', textAlign: 'center' }}>
        <div style={{ marginBottom: '12px' }}>
          <SettingsNavigator onChange={handleSettingChange} />
        </div>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', justifyContent: 'center', marginBottom: '8px' }}>
          {!isAudioReady && imageLoaded && samplerLoaded && (
            <button
              onClick={startAudio}
              style={{ padding: '12px 24px', backgroundColor: 'black', color: 'white', border: 'none', fontWeight: '600', cursor: 'pointer' }}
            >
              Start Audio & Play
            </button>
          )}
          ...
          {isAudioReady && (
            <>
              <p style={{ fontSize: '14px' }}>Audio ready, click ready</p>
              <button
                onClick={() => {
